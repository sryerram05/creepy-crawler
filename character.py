...
class Character():
    ...
    def move(self, dx, dy, obstacle_tiles, exit_tile = None):
        ...
        #logic only appliacble to player
        if self.char_type == 0:
            #check collision with player
            if exit_tile[1].colliderect(self.rect):
                #ensure player is in or near the center of the ladder
    ...
    def ai(self, player, obstacle_tiles, screen_scroll, fireball_image):
        ...
        #check if line passes through an obstacle tile
        for obstacle in obstacle_tiles:
    ...
    def update_action(self, new_action):
        #check if the new action is deifferent than the previous one
        if new_action != self.action:
            self.action = new_action
            #updte animation settings
            self.frame_index = 0
            self.update_time = pygame.time.get_ticks()
